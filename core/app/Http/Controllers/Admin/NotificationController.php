...
class NotificationController extends Controller
{
    public function global()
    {
        ...
        if (\Schema::hasColumn('general_settings', 'notification_logo') && !empty($general->notification_logo) && function_exists('getLogoIconPath')) {
            $path = getLogoIconPath() . '/' . $general->notification_logo;
            if (file_exists($path)) {
                $notificationLogoUrl = asset('assets/images/logoIcon/' . $general->notification_logo);
            }
        }
        return view('admin.notification.global_template', compact('pageTitle', 'notificationLogoUrl'));
    }
    ...
    public function globalUpdate(Request $request)
    {
        $request->validate([
            ...
            'sms_body'         => 'required',
            'notification_logo' => ['nullable', 'image', new FileTypeValidate(['jpeg', 'jpg', 'png', 'webp'])],
        ]);
        ...
        if (\Schema::hasColumn('general_settings', 'notification_logo')) {
            if ($request->has('remove_notification_logo') && $request->remove_notification_logo) {
                if (!empty($general->notification_logo) && function_exists('getLogoIconPath')) {
                    $path = getLogoIconPath() . '/' . $general->notification_logo;
                    if (file_exists($path) && is_file($path)) {
                        @unlink($path);
                    }
                }
                $general->notification_logo = null;
            }

            if ($request->hasFile('notification_logo')) {
            $path = function_exists('getLogoIconPath') ? getLogoIconPath() : public_path('assets/images/logoIcon');
            if (!is_dir($path)) {
                @mkdir($path, 0755, true);
            }
            $ext = strtolower($request->file('notification_logo')->getClientOriginalExtension() ?: 'png');
            if (!in_array($ext, ['jpg', 'jpeg', 'png', 'webp'], true)) {
                $ext = 'png';
            }
            $filename = 'notification_logo_' . md5(uniqid((string) mt_rand(), true)) . '.' . $ext;
            try {
                $request->file('notification_logo')->move($path, $filename);
                if ($general->notification_logo) {
                    $oldPath = $path . '/' . $general->notification_logo;
                    if (file_exists($oldPath) && is_file($oldPath)) {
                        @unlink($oldPath);
                    }
                }
                $general->notification_logo = $filename;
                } catch (\Throwable $e) {
                    $notify[] = ['error', 'Could not upload image: ' . $e->getMessage()];
