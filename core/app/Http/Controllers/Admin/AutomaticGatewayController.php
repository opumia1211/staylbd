...
class AutomaticGatewayController extends Controller
{
    ...
    public function update(Request $request, $code)
    {
        ...
        if (\Illuminate\Support\Facades\Schema::hasColumn($gateway->getTable(), 'sort_order')) {
            $gateway->sort_order = (int) ($request->sort_order ?? 0);
        }
        ...
        if ($request->hasFile('logo')) {
            ...
            try {
                $gateway->logo = fileUploader($request->logo, $path, getFileSize('gatewayLogo'), $gateway->logo);
            } catch (\Exception $e) {
