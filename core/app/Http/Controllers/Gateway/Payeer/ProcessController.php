...
class ProcessController extends Controller
{
    ...
    {
        if (!isset($request->m_operation_id) || !isset($request->m_sign) || !isset($request->m_orderid)) {
        ...
        $payeerAcc = json_decode($gatewayCurrency->gateway_parameter);
        ...
        $idempotencyKey = 'payeer_' . $request->m_operation_id;
        if (PaymentEventLogger::isDuplicate('Payeer', $idempotencyKey)) {
            PaymentEventLogger::logReplayAttempt('Payeer', $idempotencyKey);
        ...
        if ($request->m_amount == getAmount($deposit->final_amo) && $request->m_curr == $deposit->method_currency && $request->m_status == 'success' && $deposit->status == Status::PAYMENT_INITIATE) {
            PaymentEvent::log('Payeer', 'ipn_processed', [
