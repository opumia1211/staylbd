...
class ProcessController extends Controller
{
    ...
    public function ipn(Request $request)
    {
        ...
        if (!$orderId || !$paymentId || !$request->razorpay_signature) {
        ...
        $razorAcc = json_decode($gatewayCurrency->gateway_parameter);
        $sig = hash_hmac('sha256', $orderId . "|" . $paymentId, $razorAcc->key_secret ?? '');
        ...
        $idempotencyKey = 'razorpay_' . $paymentId;
        if (PaymentEventLogger::isDuplicate('Razorpay', $idempotencyKey)) {
            PaymentEventLogger::logReplayAttempt('Razorpay', $idempotencyKey);
        ...
        PaymentEvent::log('Razorpay', 'ipn_processed', [
