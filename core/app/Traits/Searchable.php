...
trait Searchable
{
    ...
    */
    ...
    public function scopeSearchable($query, $params, $like = true)
    {
        ...
        // Enhanced search - support single word and partial matches
        $searchTerms = explode(' ', trim($search));
        $search = $like ? "%$search%" : $search;

        $query->where(function ($q) use ($params, $search, $searchTerms) {
            foreach ($params as $key => $param) {
                $relationData = explode(':', $param);
                if (@$relationData[1]) {
                    $q = $this->relationSearch($q, $relationData[0], $relationData[1], $search, $searchTerms);
                } else {
                    $column = $param;
                    // Search for full phrase
                    $q->orWhere($column, 'LIKE', $search);
                    // Also search for individual words (for single word searches)
    ...
    private function relationSearch($query, $relation, $columns, $search, $searchTerms = [])
    {
        foreach (explode(',', $columns) as $column) {
            $query->orWhereHas($relation, function ($q) use ($column, $search, $searchTerms) {
                // Search for full phrase
