 ...
 */
    ...
    protected function getMessage()
    {
        ...
        //When shortCodes contain 'message' (admin custom notification), send ONLY that text - no email template wrapper
        if ($user && $template && isset($this->shortCodes['message']) && $this->shortCodes['message'] !== '') {
            $message = $this->shortCodes['message'];
        } elseif ($user && $template) {
            $bodyContent = $template->$body;
            $message = $this->replaceShortCode($user->fullname, $user->username, $this->setting->$globalTemplate, $bodyContent);
            if (empty($message)) {
                $message = $bodyContent ?: $template->$body;
            }
        } else {
            $message = $this->replaceShortCode($this->receiverName, $this->toAddress, $this->setting->$globalTemplate, $this->message);
        }

        // Replace short codes only when we did not use raw admin message (so template messages still get {{code}} replaced)
        if ($this->shortCodes && !(isset($this->shortCodes['message']) && $this->shortCodes['message'] !== '' && $user && $template)) {
    ...
    protected function getSubject()
    {
        ...
        if ($this->template) {
            if (isset($this->shortCodes['subject']) && $this->shortCodes['subject'] !== '') {
    ...
    public function createLog($type)
    {
        ...
        if ($this->user && $this->createLog) {
            ...
            if (!empty($this->shortCodes['link'])) {
                $notificationLog->click_url = $this->shortCodes['link'];
            }
