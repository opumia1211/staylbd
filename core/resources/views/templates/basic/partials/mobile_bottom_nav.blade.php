...
<nav class="mobile-bottom-nav pointer-events-auto fixed bottom-0 left-0 w-full z-[99999] bg-white border-t border-gray-100 shadow-[0_-2px_10px_rgba(0,0,0,0.1)]" aria-label="@lang('Mobile Navigation')" data-mobile-bottom-nav="1">
    <a href="{{ route('home', $mbNewTab) }}" class="mobile-bottom-nav__item {{ menuActive('home') }}">
        @include($activeTemplate . 'partials.header_icon_asset', ['iconKey' => 'home_icon', 'fallback' => 'home', 'width' => 19, 'height' => 19, 'alt' => ''])
        <span>@lang('Home')</span>
    </a>
    <a href="{{ route('category.all', $mbNewTab) }}" class="mobile-bottom-nav__item {{ menuActive('category*') }}">
        @include($activeTemplate . 'partials.header_icon_asset', ['iconKey' => 'categories_icon', 'fallback' => 'th-large', 'width' => 19, 'height' => 19, 'alt' => ''])
        <span>@lang('Categories')</span>
    </a>
    @auth
        <a href="{{ route('message.index', $mbNewTab) }}" class="mobile-bottom-nav__item {{ menuActive('message*') }}">
    ...
    @else
        <a href="{{ route('user.login', $mbNewTab) }}" class="mobile-bottom-nav__item {{ menuActive('user.login') }}">
            @include($activeTemplate . 'partials.header_icon_asset', ['iconKey' => 'messages_icon', 'fallback' => 'comments', 'width' => 19, 'height' => 19, 'alt' => ''])
            <span>@lang('Messages')</span>
        </a>
    @endauth
    <a href="{{ route('user.cart', $mbNewTab) }}" class="mobile-bottom-nav__item {{ menuActive('user.cart') }}">
    ...
    @auth
        <a href="{{ route('user.home', $mbNewTab) }}" id="mobile-account-btn" class="mobile-bottom-nav__item {{ menuActive('user.home') }}" aria-label="@lang('Account')">
            @include($activeTemplate . 'partials.header_icon_asset', ['iconKey' => 'login_icon', 'fallback' => 'user', 'width' => 19, 'height' => 19, 'alt' => ''])
            <span>@lang('Account')</span>
        </a>
    @else
        <a href="{{ route('guest.account.menu', $mbNewTab) }}" class="mobile-bottom-nav__item guest-account-tab-link" aria-label="@lang('Account')">
            @include($activeTemplate . 'partials.header_icon_asset', ['iconKey' => 'login_icon', 'fallback' => 'user', 'width' => 19, 'height' => 19, 'alt' => ''])
            <span>@lang('Account')</span>
        </a>
    @endauth
