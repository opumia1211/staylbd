...
@section('panel')
    <div class="row">
        <div class="col-12 mb-3">
            <a href="{{ route('admin.product.index') }}" class="btn btn-sm btn-outline--primary"><i class="las la-arrow-left"></i> @lang('Back to Products')</a>
            @if(isset($product))
                <span class="ms-2 text-muted">@lang('Product'): <strong>{{ $product->name }}</strong></span>
            @endif
        </div>
        <div class="col-md-12">
            <div class="card b-radius--10">
                <div class="card-body">
                    <form method="get" class="row g-3 mb-4 flex-wrap align-items-end">
                        @if(isset($product))
                                ...
                                <tbody>
                                    @forelse($reviews as $review)
                                        <tr>
                                            ...
                                            <td>{{ strLimit($review->title ?? '-', 30) }}</td>
                                            <td>
                                    ...
                                    @empty
                                        <tr>
                                            <td class="text-muted text-center" colspan="8">
                                                {{ __($emptyMessage ?? 'No reviews found.') }}
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
