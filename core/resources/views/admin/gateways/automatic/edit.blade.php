                        ...
                        <div class="payment-method-item block-item">
                            <div class="payment-method-header">
                                ...
                                <div class="content ps-0 w-100">
                                    ...
                                    <div class="mt-3">
                                        <label class="form-label fw-bold">@lang('Official Logo')</label>
                                        <p class="small text-muted mb-2">@lang('Upload the payment method official logo. It will be shown on the deposit/payment selection page.')</p>
                                        <div class="d-flex align-items-center gap-3 flex-wrap">
                                            @if($gateway->logo ?? null)
                                                <div class="border rounded p-2 bg-light">
                                                    <img src="{{ getGatewayLogoUrl($gateway->alias, $gateway->logo) }}" alt="{{ __($gateway->name) }}" style="max-height: 48px; width: auto;" class="img-fluid"/>
                                                </div>
                                            @endif
                                            <input type="file" class="form-control form-control-sm w-auto" name="logo" accept="image/png,image/jpeg,image/jpg,image/webp"/>
                                        </div>
                                    </div>
                                    @if(\Illuminate\Support\Facades\Schema::hasColumn('gateways', 'sort_order'))
                                    <div class="mt-3">
                                        <label class="form-label fw-bold">@lang('Sort Order')</label>
                                        <input type="number" class="form-control form-control-sm w-auto" name="sort_order" value="{{ old('sort_order', $gateway->sort_order ?? 0) }}" min="0" style="max-width: 100px;"/>
                                        <p class="small text-muted mb-0">@lang('Lower number appears first on payment page.')</p>
                                    </div>
                                    @endif
                                </div>
                            </div>
