...
Route::group(['prefix' => '{locale?}', 'where' => ['locale' => 'en|bn|hi|ar|ur|ru|zh|es|fr|de|pt|ja']], function () {
    ...
    Route::controller('SiteController')->group(function () {
        ...
        Route::get('/contact/chat-messages', 'getChatMessages')->name('contact.chat.messages');
        Route::get('/contact/chat-unread-count', 'getChatUnreadCount')->name('contact.chat.unread');
...
// Real-time product polling (fallback for WebSockets)
Route::get('/realtime/product/{id}', [App\Http\Controllers\Storefront\RealtimePollController::class, 'product'])->name('storefront.realtime.product');

// Non-localized or Special Routes
Route::get('ticket', function () { return redirect()->route('message.index', ['locale' => session('locale', 'en')], 301); });
Route::get('wish-list/product', function () { return redirect()->route('user.wishlist', ['locale' => session('locale', 'en')], 301); });
Route::get('cart-list/product', function () { return redirect()->route('user.cart', ['locale' => session('locale', 'en')], 301); });
Route::get('compare', function () { return redirect()->route('user.compare', ['locale' => session('locale', 'en')], 301); });
